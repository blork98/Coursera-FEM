...
void FEM<dim>::define_boundary_conds() {
	...
	double drichletBC = 0.0;
	double boundaryLocationX3 = 0.0;

	unsigned int x3;

	for (unsigned int globalDOF = 0; globalDOF < totalDOFs; globalDOF++)
	{
		//x3 = dofLocation[globalDOF][2];
		//if ( std::fabs(x3 - 0.0) < 1e-8)
						...
						for (unsigned int k = 0; k<dim; k++) {
							for (unsigned int j = 0; j<dim; j++) {
								for (unsigned int l = 0; l<dim; l++) {
									...
									basis gradient vector: fe_values.shape_grad(elementDOF,q)[j], where elementDOF is dim*A+i or dim*B+k
									...
									Klocal[dim*A + i][dim*B + k] += fe_values.JxW(q)*fe_values.shape_grad(dim*A + i,q)[j] * C(i, j, k, l) * fe_values.shape_grad(dim*B + k,q)[l];
								}
							}
						}
					}
