...
double FEM<dim>::basis_function(unsigned int node, double xi_1, double xi_2, double xi_3) {
	...
	double value = 0.; //Store the value of the basis function in this variable
					   ...
					   //EDIT
	if (node == 0)
		value = (1.0 / 8.0)*(1-xi_1)*(1-xi_2)*(1-xi_3);
	else if (node == 1)
		value = (1.0 / 8.0)*(1 + xi_1)*(1 - xi_2)*(1 - xi_3);
	else if (node == 2)
		value = (1.0 / 8.0)*(1 - xi_1)*(1 + xi_2)*(1 - xi_3);
...
std::vector<double> FEM<dim>::basis_gradient(unsigned int node, double xi_1, double xi_2, double xi_3) {
	...
	if (node == 0)
	{
		values[0] = -(1.0/8.0)*(1 - xi_2)*(1 - xi_3);
		values[1] = -(1.0 / 8.0)*(1 - xi_1)*(1 - xi_3);
		values[2] = -(1.0 / 8.0)*(1 - xi_1)*(1 - xi_2);
	}
	else if (node == 1)
	{
		values[0] = (1.0 / 8.0)*(1 - xi_2)*(1 - xi_3);
...
void FEM<dim>::define_boundary_conds() {
	...
	double x_min = 0.0, x_max = 0.04, 
		y_min = 0.0, y_max = 0.08, 
		z_min = 0.0, z_max = 0.02;
	...
	for (unsigned int globalNode = 0; globalNode < totalNodes; globalNode++)
	{
		...
		y = nodeLocation[globalNode][1];
		z = nodeLocation[globalNode][2];
		if (std::fabs(nodeLocation[globalNode][0] - x_min) < 1e-8)
			boundary_values[globalNode] = 300.0*(1.0 + (1.0 / 3.0)*(y + z));
		else if (std::fabs(nodeLocation[globalNode][0] - x_max) < 1e-8)
			boundary_values[globalNode] = 310.0*(1.0 + (1.0 / 3.0)*(y + z));
	}
}
								...
								for (unsigned int j = 0; j<dim; j++) {
									for (unsigned int I = 0; I<dim; I++) {
										for (unsigned int J = 0; J<dim; J++) {
											...
											dNA_dXI = basis_gradient(A, quad_points[q1], quad_points[q2], quad_points[q3])[i] * invJacob[i][I];
											dNB_dXJ = basis_gradient(B, quad_points[q1], quad_points[q2], quad_points[q3])[j] * invJacob[j][J];
											Klocal[A][B] += detJ*quad_weight[q1] * quad_weight[q2]* quad_weight[q3] * kappa[i][j] * dNA_dXI* dNB_dXJ;
										}
									}
								}
							}
...
void FEM<dim>::solve() {
	...
	//Use CG method -- could not get UMFPACK for windows vs
