...
FEM<dim>::FEM(unsigned int order, unsigned int problem)
	:
	fe(FE_Q<dim>(QIterated<1>(QTrapez<1>(), order)),dim), //fe(FE_Q<dim>(order), dim),
...
double FEM<dim>::basis_gradient(unsigned int node, double xi) {
	...
	{
		if (node == 0)
			value = 0.5*(2.0*xi - 1.0);
		else if (node == 2)
			value = -2.0*xi;
		else if (node == 1)
			value = 0.5*(1.0 + 2.0*xi);
	}
...
void FEM<dim>::setup_system() {
	...
	//polynomial = 2*quad_rule -1 
	//max polynomial for norm is (3)2 = 6 there fore 6 = 2*q-1 q = (6+1)/2 = 4
	...
	quad_points[0] = -0.8611363116; //EDIT
	quad_points[1] = -0.3399810436; //EDIT
	quad_points[2] = 0.3399810436;
	quad_points[3] = 0.8611363116;

	quad_weight[0] = 0.3478548451; //EDIT
	quad_weight[1] = 0.6521451549; //EDIT
	...
	quad_points[0] = 0.0000000; //EDIT
	quad_points[1] = -0.7745966692414834; //EDIT
	quad_points[2] = 0.7745966692414834;
	
	quad_weight[0] = 0.8888888888888888; //EDIT
	quad_weight[1] = 0.5555555555555556; //EDIT
	quad_weight[2] = 0.5555555555555556;
	*/
	//QuadRule = 2
	/*	
	...
	for (; elem != endc; ++elem) {
		...
		for (unsigned int A = 0; A<dofs_per_elem; A++) {
			for (unsigned int q = 0; q<quadRule; q++) {
				...
				//EDIT - Define Flocal.
				Flocal[A] += quad_weight[q] * basis_function(A, quad_points[q])*x;
				//Flocal[A] *= 0.5*h_e*std::pow(10,11)*std::pow(10,-4);
			}
			Flocal[A] *= 0.5*h_e*std::pow(10,11)*std::pow(10,-4);
			//std::cout << Flocal[A] << std::endl;
		}
		...
		for (unsigned int A = 0; A<dofs_per_elem; A++) {
			for (unsigned int B = 0; B<dofs_per_elem; B++) {
				for (unsigned int q = 0; q<quadRule; q++) {
					//EDIT - Define Klocal.
					Klocal[A][B] += quad_weight[q] * basis_gradient(A, quad_points[q])*basis_gradient(B, quad_points[q]);
				}
				Klocal[A][B] *= std::pow(10,11)*std::pow(10,-4)*2.0 / h_e;
				//std::cout << Klocal[A][B] << std::endl;
			}
...
double FEM<dim>::l2norm_of_error() {
	...
	for (; elem != endc; ++elem) {
		...
		for (unsigned int q = 0; q<quadRule; q++) {
			...
			if (prob == 1)
				u_exact = -(1.0 / 6.0)*x*x*x + x*( ((g2-g1)/L) + ((L*L)/6.0) ) + g1;
			else if (prob == 2)
				u_exact = -(1.0 / 6.0)*x*x*x + x*(0.1+0.5*L*L) + g1;

			l2norm += (u_exact-u_h)*(u_exact - u_h)*0.5*h_e*quad_weight[q];
